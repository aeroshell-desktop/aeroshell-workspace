    ...
    , m_window(false)
{
    ...
    if (m_theme.isEmpty()) {
        KConfigGroup ksplashCfg = KSharedConfig::openConfig("ksplashrc")->group(QStringLiteral("KSplash"));
        if (ksplashCfg.readEntry("Engine", QStringLiteral("AeroShell")) == QLatin1String("AeroShell")) {
...
void SplashApp::timerEvent(QTimerEvent *event)
{
    if (event->timerId() == m_timer.timerId()) {
        ...
        setStage(QString::number(m_test + 1));
...
{
    if(m_stages.contains(stage)) return;
...
{
    if(screen->geometry().isNull()) return;

    auto *w = new SplashWindow(m_testing, m_window, m_theme, screen);
    w->setGeometry(screen->geometry());
    for(const QString &stage : std::as_const(m_stages)) {
        w->setStage(stage);
    }
    w->setVisible(true);
    m_windows << w;
...
{
    if(!KWindowSystem::isPlatformWayland()) return;
